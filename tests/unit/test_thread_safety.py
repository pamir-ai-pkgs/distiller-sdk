...
class TestDisplayThreadSafety:
    ...
    def test_display_has_lock(self, tmp_path):
        ...
        assert isinstance(display._lock, type(threading.Lock())), "Display._lock should be a threading.Lock"
...
class TestLEDThreadSafety:
    ...
    def test_led_concurrent_color_changes(self, mock_led_hardware):
        ...
        threads = [
            threading.Thread(target=set_color_task, args=(0, i * 50, 0, 0))
            for i in range(5)
        ]
        for t in threads:
            t.start()
...
class TestParakeetThreadSafety:
    ...
    def test_parakeet_has_lock(self, mock_parakeet_models, monkeypatch):
        ...
        with patch('distiller_sdk.parakeet.parakeet.Audio'):
            parakeet = Parakeet(
                model_config={"model_hub_path": str(mock_parakeet_models)},
                configure_audio=False
            )

        assert hasattr(parakeet, '_lock'), "Parakeet should have a _lock attribute"
        assert isinstance(parakeet._lock, type(threading.Lock())), "Parakeet._lock should be a threading.Lock"
...
class TestWhisperThreadSafety:
    ...
    def test_whisper_has_lock(self, mock_whisper_models):
        ...
        with patch('distiller_sdk.whisper.fast_whisper.WhisperModel'), \
             patch('distiller_sdk.whisper.fast_whisper.Audio'):
            whisper = Whisper(
                model_config={"model_hub_path": str(mock_whisper_models)},
                configure_audio=False
            )

        assert hasattr(whisper, '_lock'), "Whisper should have a _lock attribute"
        assert isinstance(whisper._lock, type(threading.Lock())), "Whisper._lock should be a threading.Lock"
...
class TestAudioThreadSafetyVerification:
    ...
    def test_audio_has_lock(self):
        ...
        assert isinstance(audio._lock, type(threading.Lock())), "Audio._lock should be a threading.Lock"
...
class TestCameraThreadSafetyVerification:
    ...
    def test_camera_has_frame_lock(self, mock_camera_hardware):
        ...
        assert isinstance(camera._frame_lock, type(threading.Lock())), "Camera._frame_lock should be a threading.Lock"
