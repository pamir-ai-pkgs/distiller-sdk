...
class TestAudioDetection:
    ...
    def test_get_status_unavailable_hardware(self, mock_audio_unavailable):
        ...
        assert "found" in status.message.lower() or "unavailable" in status.message.lower() or "not installed" in status.message.lower()
    ...
    def test_get_status_caches_device_list(self, mock_audio_hardware):
        ...
        assert 'input_devices' in status.diagnostic_info or 'output_devices' in status.diagnostic_info or 'devices' in status.diagnostic_info

    @pytest.mark.parametrize("command_error", [
    ...
    def test_get_status_handles_various_errors(self, monkeypatch, command_error):
        ...
        assert isinstance(status.error, (FileNotFoundError, PermissionError, subprocess.CalledProcessError))
